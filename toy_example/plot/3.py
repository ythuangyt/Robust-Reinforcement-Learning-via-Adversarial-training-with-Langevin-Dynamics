...
if __name__ == '__main__':
	...
	#plt.plot(xlist,x1,label='x1_EG')
	#plt.plot(xlist,y1,label='y1_EG')
	plt1.plot(xlist,np.mean(d_SGLD2,axis=0),label='MixedNE-LD',marker=">",color='red',markevery=5)
	#plt.plot(xlist,x2,label='x2_SGLD')
	#plt.plot(xlist,y2,label='y2_SGLD')
	...
	xx = np.arange(0.4,1.5,0.2)
	yy = np.arange(0,1,0.2)
	X, Y = np.meshgrid(xx, yy)
	u = gx(X,Y)
	v = -gy(X,Y)
	plt2.quiver(X,Y,u,v,scale=20,color='gray',width=0.005,minlength=2,alpha=0.5)





	x=np.arange(0.3,1.6,0.05)
	y=0*x 
